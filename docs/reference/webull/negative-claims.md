...
| No market-status or trading-calendar endpoint on the OpenAPI surface this package can reach | inventory (85 endpoints, none named `market-status`/`trading-calendar`), live sitemap re-check, 2026-09-07 | **holds, with a nuance worth recording rather than flattening away.** Webull DOES publish one trading-calendar endpoint — `GET /broker/master-data/trading-calendars/list` — but it belongs to the **Broker API**, a separate product for building a white-label brokerage on Webull's infrastructure (`developer.webull.com/apis/docs/broker-api/about-broker-api.md`), served from `broker-api.webull.com` rather than this package's `api.webull.com`, requiring its own broker-tier credential obtained through a separate business application ("contact our business team"). This package's `credentials()` is an OpenAPI App Key/App Secret pair and does not model that credential; this package has no relationship that could supply one. The claim "this package cannot answer market status" holds; the stronger claim "Webull publishes no such endpoint anywhere" would have been the same one-level-too-broad mistake this file opens by naming |
