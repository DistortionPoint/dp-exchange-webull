defmodule DpExchange.Webull.FeedTest do
  @moduledoc """
  Shard bookkeeping, coverage, and what reaches a subscriber.

  The three slowest areas — the periodic re-assert, the blind-resubscribe identity rules,
  and the busy-feed call timeout — live in files of their own so their waiting overlaps
  rather than stacking up; see `DpExchange.Webull.FeedCase`.
  """
  ...
  describe "a reconcile task that never answers does not strand its caller" do
    ...
    test "a timer for a reconcile that already answered is ignored" do
      ...
      # A ref of its own, because the deadline now names the attempt rather than the tag.
      # Nothing is tracked here at all, so this is the "already answered" arm.
      send(feed, {:reconcile_timeout, {:background, 0}, make_ref()})
      _settled = Feed.coverage(feed)
